...
double get_r_max(double m_Q) { return R_RANGE_FACTOR / m_Q; }

void import_interp_data_by_params(
    const Interpolator3D::DataGenerationConfig* config) {
  std::string filepath;

  std::string m_path = std::to_string(int(m * 100));
  std::string rH_sqr_path =
      (rH_sqr >= 1.0) ? std::to_string(int(std::round(rH_sqr * 100)))
                      : "0" + std::to_string(int(std::round(rH_sqr * 100)));
...
void set_parameters(int argc, char** argv) {
  ...
  const std::string error_message =
      ...
      "\t[-m <gluon mass>]\n"
      "\t[--g2mu02-factor <number>] (multiplies base value of g2mu02 by "
      "<number>)\n"
  ...
  for (int i = 1; i < argc; i += 2) {
    ...
    if (flag.str() == "-p") {
      ...
      quark_output_config = Quark::Bottom;
      ...
      quark_output_config = Quark::Charm;
    ...
    if (!(arg >> arg_number) || !arg.eof()) {
      std::cerr << "You entered the argument \"" << arg.str()
                << "\" for the option " << "\"" << flag.str()
...
void print_infos(std::ofstream& out, uint seed, const HotspotNucleus& nucleus) {
  ...
  for (uint n = 0, n_max = nucleus.get_atomic_num(); n < n_max; n++) {
    ...
    for (uint i = 0, i_max = nucleus.get_num_hotspots_per_nucleon(); i < i_max;
         i++) {
...
uint get_unique_seed() {
  ...
  if (seed == 0) seed = hash(thread_id_temp) * GET_PROCESS_ID() - time(0);
...
std::string get_default_filepath_from_parameters() {
  ...
  filepath += "g2mu02/";
#endif
  filepath += (char)quark_output_config;

#ifndef _G2MU02
  filepath +=
      (g_g2mu02_config_factor >= 1.0)
          ? std::to_string(int(std::round(g_g2mu02_config_factor * 10.0)))
          : "0" +
                std::to_string(int(std::round(g_g2mu02_config_factor * 10.0)));
#endif
  filepath +=
...
double sin_zeros(uint n) { return double(n) * PI; }

double cos_zeros(uint n) { return (-0.5 + double(n)) * PI; }
