...
double dsigma_d2b_sqr(double x1, double x2, double y1, double y2, double xb1,
  ...
  if (Sqrt == 0.0)
    ...
    DD = 0.5 * ((1.0 + factor) * exp((a + d + Sqrt) * 0.5) +
                (1.0 - factor) * exp((a + d - Sqrt) * 0.5));
  }

  return 4.0 * (1.0 - D(G_xy) - D(G_xbyb) + DD);
#else
  return 4.0 * (G_xy * G_xbyb +
                sqr(G_xxb + G_yyb - G_xyb - G_yxb) /
                    2.0);  // probably wrong (but also not in use) but needs to
                           // be checked (factor of 1/8 probably missing)
#endif
}
...
#else
  return 4.0 * (G_xy * G_xbyb +
                0.5 * Ncsqrm1_inverse * sqr(G_xxb + G_yyb - G_xyb - G_yxb));
#endif
}
...
double dsigma_d2b_sqr_reduced(double x1, double x2, double y1, double y2,
  ...
  double avg = 0.0;
  for (uint i = 0; i < InternalHotspotAvg::num; ++i)
    avg += Sampled::dsigma_d2b_sqr_reduced(x1, x2, y1, y2, xb1, xb2, yb1, yb2,
                                           &hn[i]);
...
double sch_sbch(double x1, double x2, double y1, double y2, double xb1,
                double xb2, double yb1, double yb2) {
  double avg_sc = 0.0;
  double avg_sbc = 0.0;
  for (uint i = 0; i < InternalHotspotAvg::num; i++) {
...
// < <sigma sigmabar>c >h
double ssbch(double x1, double x2, double y1, double y2, double xb1, double xb2,
...
double scsbch(double x1, double x2, double y1, double y2, double xb1,
              double xb2, double yb1, double yb2) {
  double avg = 0.0;
  for (uint i = 0; i < InternalHotspotAvg::num; i++)
    avg += Sampled::dsigma_d2b(x1, x2, y1, y2, &hn[i]) *
           Sampled::dsigma_d2b(xb1, xb2, yb1, yb2, &hn[i]);
