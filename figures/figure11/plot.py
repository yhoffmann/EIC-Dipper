...
sub0.set_ylabel(
    r'${\rm d}\sigma_{\rm incoh}/{\rm d}t \times (\Gamma_{{\rm J}/\Psi} / \Gamma_{\rm V}) \times (m_{\rm q}/m_{\rm c})^5 ~[{\rm nb~GeV}^{-2}]$')
...
sub0.plot(t, C_FACTOR*sdic10['Inco'],
          label='Dilute, charm', color=SCHEME[8], linestyle=':')
sub0.fill_between(t, C_FACTOR*err(sdic10['Inco']-sdic10['IncoErr']), C_FACTOR*err(
    sdic10['Inco']+sdic10['IncoErr']), color=SCHEME[8], alpha=0.4)

sub0.plot(t, C_FACTOR*sdec10['Inco'], label='Dense, charm', color=SCHEME[8])
sub0.fill_between(t, C_FACTOR*err(sdec10['Inco']-sdec10['IncoErr']), C_FACTOR*err(
    sdec10['Inco']+sdec10['IncoErr']), color=SCHEME[8], alpha=0.4)

sub0.plot(t, B_FACTOR*sdib10['Inco'],
          label='Dilute, bottom', color=SCHEME[1], linestyle=':')
sub0.fill_between(t, B_FACTOR*err(sdib10['Inco']-sdib10['IncoErr']), B_FACTOR*err(
    sdib10['Inco']+sdib10['IncoErr']), color=SCHEME[1], alpha=0.4)

sub0.plot(t, B_FACTOR*sdeb10['Inco'], label='Dense, bottom', color=SCHEME[1])
sub0.fill_between(t, B_FACTOR*err(sdeb10['Inco']-sdeb10['IncoErr']), B_FACTOR*err(
    sdeb10['Inco']+sdeb10['IncoErr']), color=SCHEME[1], alpha=0.4)
...
sub0ratio.plot(t, 1.0/(sdib10['Inco']/sdeb10['Inco']),
               color=BOTTOM_COLOR, linestyle='--')

fig0.savefig('figure11.pdf', bbox_inches='tight')
